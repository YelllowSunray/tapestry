...
import LeavesStatusForm from '@/app/components/LeavesStatusForm';
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      const { data: postsData, error: postsError } = await client
        .from('posts')
        .select('id, content, created_at, user_id, category, category_emoji, subcategory, photo_url')
        .eq('section', 'leaves')
          ...
          <div className="space-y-8">
            <div className="max-w-2xl mx-auto">
              <h1 className="text-xl text-center text-green-600 dark:text-green-400 mb-8">
                Celebrate the present — your vibes, meals, music, casual updates, and beauty in daily life.
              </h1>
            </div>
            {session.user && <LeavesStatusForm onPostAdded={fetchPosts} />}
            ...
            <div className="max-w-2xl mx-auto">
              ...
              {!loadingPosts && posts.length === 0 && session.user && (
                <p className="text-center text-emerald-600 dark:text-emerald-300 font-medium">No posts yet. Be the first to share your daily moments!</p>
              )}
              {!session.user && (
